...
export function ClubForm({ action, deleteAction, defaultValues }: ClubFormProps) {
    ...
    <div className="flex flex-col gap-px">
      <form action={action}>
        ...
        <Section label="Stammdaten">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Sportart">
              <select
                name="sport"
                defaultValue={defaultValues?.sport?.toLowerCase() ?? ""}
        ...
        <Section label="Social Media">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Instagram">
              <input name="social_instagram" defaultValue={defaultValues?.social_instagram ?? ""} className={inputClass} placeholder="@vereinsname" />
            </Field>
            <Field label="Facebook">
              <input name="social_facebook" defaultValue={defaultValues?.social_facebook ?? ""} className={inputClass} placeholder="facebook.com/vereinsname" />
            </Field>
          </div>
        </Section>
        ...
        <Section label="Details">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Gründungsjahr">
              <input name="founded_year" type="number" min={1800} max={2100} defaultValue={defaultValues?.founded_year ?? ""} className={inputClass} placeholder="z. B. 1952" />
            </Field>
            <Field label="Mitgliederzahl">
              <input name="members_count" type="number" min={0} defaultValue={defaultValues?.members_count ?? ""} className={inputClass} placeholder="z. B. 250" />
            </Field>
          </div>
        </Section>
        ...
        <Section label="Einstellungen">
          <Field label="Status">
            <select name="status" defaultValue={defaultValues?.status ?? "approved"} className={inputClass}>
          ...
          <Field label="Homepage Spotlight">
            <label className={`flex items-center justify-between gap-4 px-4 py-3 border cursor-pointer transition-all ${defaultValues?.is_spotlight ? "border-tertiary bg-tertiary/5" : "border-outline-variant/20 hover:border-outline-variant/40"}`}>
              <div>
                ...
                <p className="text-[10px] text-on-surface-variant mt-0.5">Erscheint im Spotlight-Bereich auf der Startseite</p>
              </div>
              <input
                type="checkbox"
                name="is_spotlight"
          ...
          <Field label="Header-Bild">
            <ImageField name="hero_image" currentUrl={defaultValues?.hero_image_url ?? null} />
          </Field>
          <Field label="Logo">
            <ImageField name="logo" currentUrl={defaultValues?.logo_url ?? null} />
          </Field>
        </Section>
        ...
        <div className="bg-surface-container-lowest px-8 py-5 flex items-center gap-3 border-t border-outline-variant/10">
          ...
          <Link href="/admin/sport" className="px-5 py-2.5 font-bold uppercase text-xs tracking-widest text-on-surface-variant hover:text-primary transition-colors">
            Abbrechen
          </Link>
        </div>
      </form>
...
function ImageField({ name, currentUrl }: { name: string; currentUrl: string | null }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <input type="hidden" name={`${name}_url`} value={preview && preview.startsWith("http") ? preview : ""} />
      <input
        ref={inputRef}
        type="file"
        name={`${name}_file`}
