...
export default async function QueuePage({
  ...
  const [{ count: pendingBiz }, { count: pendingEv }] = await Promise.all([
  ...
  const pendingCount = (pendingBiz ?? 0) + (pendingEv ?? 0);

  const totalItems = (businesses?.length ?? 0) + (events?.length ?? 0);
