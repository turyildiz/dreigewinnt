...
interface GalleryManagerProps {
  ...
  deleteAction: (photoId: string, businessId: string) => Promise<void>;
}

export function GalleryManager({ businessId, photos, limit, uploadAction, deleteAction }: GalleryManagerProps) {
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  const canUpload = limit === Infinity || photos.length < limit;
  const limitLabel = limit === Infinity ? "unbegrenzt" : `${photos.length} / ${limit}`;
  ...
  function handleDelete(photoId: string) {
    startTransition(async () => {
      await deleteAction(photoId, businessId);
    });
  }
  ...
  return (
    <div className="mt-6 bg-surface-container-lowest">
      <div className="px-8 pt-6 pb-1 border-b border-outline-variant/10">
        ...
        <p className="text-[10px] text-on-surface-variant mt-0.5">{limitLabel} Fotos</p>
      </div>
