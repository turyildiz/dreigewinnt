...
import { createClubPostAction, deleteClubPostAction, updateClubPostAction, removeClubPostImageAction } from "./actions";
...
export default async function ClubPostsPage({
  ...
  const [{ data: club }, { data: posts }] = await Promise.all([
    supabaseAdmin.from("clubs").select("id, name").eq("id", id).single(),
    supabaseAdmin
      .from("club_posts")
  ...
  const createAction = createClubPostAction.bind(null, id);
          ...
          <form action={createAction} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                placeholder="What's new at this club?"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">
        ...
        <div>
          ...
          {!posts || posts.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {posts.map((post) => {
                const deleteAction = deleteClubPostAction.bind(null, post.id, id);
                const updateAction = updateClubPostAction.bind(null, post.id, id);
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        {isEditing ? (
                          ...
                          <Link
                            href={`/admin/sport/${id}/posts?edit=${post.id}`}
                            className="text-on-surface-variant/40 hover:text-primary transition-colors"
                          >
                        ...
                        {images.length > 0 && (
                          <div className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {images.map((url, i) => {
                                const removeAction = removeClubPostImageAction.bind(null, post.id, id, url);
                                return (
