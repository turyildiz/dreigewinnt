...
export async function submitBusinessAction(
  ...
  if (isPaid) {
    ...
    if (heroFile && heroFile.size > 0) {
      heroImageUrl = await uploadToStorage(heroFile, "business-images", `submissions/${slug}`);
    }
    if (isPremium) {
      const galleryFiles = formData.getAll("gallery_images") as File[];
      for (const file of galleryFiles) {
        if (file && file.size > 0) {
          const url = await uploadToStorage(file, "business-images", `submissions/${slug}/gallery`);
  ...
  if (biz && galleryUrls.length > 0) {
    await supabaseAdmin.from("business_photos").insert(
      galleryUrls.map((url, i) => ({ business_id: biz.id, photo_url: url, sort_order: i }))
    );
  }
