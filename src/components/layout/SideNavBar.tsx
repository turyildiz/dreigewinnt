        ...
        {section === "/sport" && (
          <>
            ...
            {SPORT_CATEGORIES.map((sport) => {
              const isSportActive = activeSport === sport.value;
              const href = activeTown
                ? `${section}?town=${activeTown}&sport=${sport.value}`
                : `${section}?sport=${sport.value}`;
              return (
                <Link
                  key={sport.value}
                  href={href}
      ...
      <div className="mt-auto px-8 pt-6">
        {section === "/sport" ? (
          <div className="editorial-gradient p-6 rounded shadow-lg text-on-secondary">
            <p className="text-[10px] font-bold tracking-widest uppercase mb-2">Sportverein</p>
            <h4 className="font-headline font-bold text-lg leading-tight mb-4">Verein eintragen</h4>
            <Link href="/sport/einreichen" className="block w-full bg-white text-secondary py-2 text-[12px] font-bold uppercase tracking-wider rounded-sm hover:bg-surface transition-colors text-center">
              Jetzt Eintragen
            </Link>
          </div>
        ) : (
